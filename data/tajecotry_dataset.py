...
from data.standard_trajectory_encoder import StandardTrajectoryEncoder
import data.utils
...
class TrajectoryDataset(AbstractDataset):
    ...
    def __init__(self, config):
        ...
        self.cut_data_cache = './cache/dataset_cache/cut_traj'
    ...
    def get_data(self):
        ...
        return data.utils.generate_dataloader_pad(train_data, eval_data, test_data,
    ...
    def cutter_filter(self):
        ...
        filter_location = group_location[group_location['time']
                                         >= self.config['min_checkins']]
        ...
        if cut_method == 'time_interval':
            ...
            for uid in tqdm(user_set, desc="cut and filter trajectory"):
                ...
                for index, row in enumerate(usr_traj):
                    now_time = data.utils.parse_time(row[2])
                    if index == 0:
                        session.append(row.tolist())
                        prev_time = now_time
                    else:
                        time_off = data.utils.cal_timeoff(now_time, prev_time)
            ...
            if max_session_len != window_size:
                raise ValueError(
                    'the fixed length window is not equal to max_session_len')
            for uid in tqdm(user_set, desc="cut and filter trajectory"):
                usr_traj = traj[traj['entity_id'] == uid].to_numpy()
                sessions = []  # 存放该用户所有的 session
                session = []  # 单条轨迹
    ...
    def get_encoder(self):
        try:
            return StandardTrajectoryEncoder(self.config)
